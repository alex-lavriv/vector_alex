fn main() {
    ...
    assert_eq!(my_vector.capacity(), 16);
...
struct VectorAlex<T> {
    len: usize,
    cap: usize,
    items_1: Option<*mut u8>,
    phantom: std::marker::PhantomData<T>,
}

impl<T: Default + Copy + Clone> VectorAlex<T> {
    pub fn new() -> VectorAlex<T> {
        VectorAlex { len: 0, cap: 0, items_1: None, phantom: std::marker::PhantomData }
    }
    ...
    fn set_item(&mut self, item: T, index: usize) {
        if let Some(items) = self.items_1 {
            Self::set_item_ptr(items, item, index)
        }
    }
    ...
    pub fn get_item(&mut self, index: usize) -> T {
            if let Some(items) = self.items_1 {
               return Self::get_item_ptr(items, index);
            }
            Default::default()
    }
    fn reallloc(&mut self) {
        unsafe {
            if let Some(prev_items) = self.items_1 {
                ...
                // copy prev arr
                for i in 0..self.len {
                    let prev_val = Self::get_item_ptr(prev_items, i);
                    Self::set_item_ptr(ptr, prev_val, i);
                }
                self.items_1 = Some(ptr);
            } else {
                self.cap = 4;
